...
const routes = resource({
	...
	loader: async (parameter) => {
        const [result, count] = await surreal.up().then(() => surreal.query<[Route[], number]>(`SELECT *, designations.{type.*, number}, timetable.* FROM route ${parameter.search ? 'WHERE name.lowercase().starts_with($search.lowercase())' : ''} START ($page - 1) * $size LIMIT $size; (SELECT count() FROM route ${parameter.search ? 'WHERE name CONTAINS $search' : ''} GROUP ALL)[0].count`, parameter))
...
const edit = resource({
    parameter: { route },
	loader: async (parameter) => new RouteEditDto(parameter.route.params.id === 'new' ? {} : await surreal.up().then(() => surreal.select(new RecordId('route', parameter.route.params.id))))
})
