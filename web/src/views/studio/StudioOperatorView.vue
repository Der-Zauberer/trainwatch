...
const operators = resource({
	...
	loader: async (parameter) => {
        const [result, count] = await surrealdb.query<[Operator[], number]>(`SELECT * FROM operator ${parameter.search ? 'WHERE name.lowercase().starts_with($search.lowercase())' : ''} START ($page - 1) * $size LIMIT $size; (SELECT count() FROM operator ${parameter.search ? 'WHERE name CONTAINS $search' : ''} GROUP ALL)[0].count`, parameter)
