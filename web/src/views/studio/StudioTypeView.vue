...
const types = resource({
	...
	loader: async (parameter) => {
        const [result, count] = await surreal.up().then(() => surreal.query<[Type[], number]>(`SELECT * FROM type ${parameter.search ? 'WHERE name.lowercase().starts_with($search.lowercase())' : 'ORDER BY priority'} START ($page - 1) * $size LIMIT $size; (SELECT count() FROM type ${parameter.search ? 'WHERE name CONTAINS $search' : ''} GROUP ALL)[0].count`, parameter))
...
const edit = resource({
    parameter: { route },
	loader: async (parameter) => new TypeEditDto(parameter.route.params.id === 'new' ? {} : await surreal.up().then(() => surreal.select(new RecordId('type', parameter.route.params.id))))
})
