...
class SwdElementRef {
  ...
  isCommentHidden = () => {
    return (
      this.#swdElementRef.innerHTML.trim().startsWith('<!--') &&
      this.#swdElementRef.innerHTML.trim().endsWith('-->')
    )
  }
  commentToggle = () => {
...
class SwdNavigation extends SwdComponent {
  ...
  static autoHide(event) {
    ...
    if (this.#ignoreNextHide) {
      this.#ignoreNextHide = !1
      return
    }
    if (
      event &&
      SwdNavigation.#shownNavigation.contains(event.target) &&
      !(event.target.nodeName == 'A' && event.target.hasAttribute('href'))
    ) {
      return
    }
...
class SwdDropdown extends SwdComponent {
  ...
  #INPUT_EVENT = (event) => {
    if (this.isHidden()) this.show()
    if (this.#selection && this.#dropdownInput && !this.#dropdownInput.hasAttribute('readonly'))
      this.#selection.filter(event.target.value)
  ...
  swdOnInit() {
    ...
    this.swdRegisterManagedEvent(this, 'keydown', (event) => {
      ...
      switch (event.key) {
        ...
        case 'Delete':
        case 'Backspace':
          if (this.#dropdownInput && this.#dropdownInput.hasAttribute('readonly'))
  ...
  show() {
    ...
    if (this.#selection && this.#dropdownInput && !this.#dropdownInput.hasAttribute('readonly'))
      this.#selection.filter(this.#dropdownInput.value)
    this.#setDropdownDirectionAndSize()
  }
  hide() {
    if (!this.#dropdownContent) return
  ...
  isHidden() {
    if (!this.#dropdownContent) return !0
  ...
  #setDropdownDirectionAndSize() {
    ...
    if (contentRect.right > window.innerWidth)
      this.#dropdownContent.classList.add('swd-dropdown-content-right')
...
class SwdSelection extends SwdComponent {
  ...
  next() {
    this.#nextOrPrevious(!0, !1)
  }
  previous() {
    this.#nextOrPrevious(!1, !1)
  }
  #nextOrPrevious(next, first) {
    ...
    if (targetOffset + taregtRect.height > content.scrollTop + contentRect.height)
      content.scrollTop = targetOffset + taregtRect.height - contentRect.height
  ...
  select(target) {
    ...
    if (!targetToSelect || targetToSelect.nodeName !== 'A' || targetToSelect.hasAttribute('hidden'))
      return
    this.selected = targetToSelect
    this.value = this.selected.getAttribute('value') || this.selected.innerText
    this.dispatchEvent(new Event('select'))
    if (this.#selectionChangeAction)
      this.#selectionChangeAction(this.selected.innerText, this.value)
  }
  setOnSelect(action) {
  ...
  filter(text) {
    ...
    for (const element of this.children) {
      let isElementVisible = !1
      const elementTextParts = []
      let currentTextPart = element.innerHTML
        .toLowerCase()
        .replace(/<\/?[^>]*>/g, ' ')
        .replace(/[^\w\d\s]/gm, '')
        .replace(/\s{2,}/g, ' ')
...
class SwdCode extends SwdComponent {
  ...
  #highlight() {
    switch (this.getAttribute('swd-code-language')) {
      case 'html':
        this.#shadowDom.innerHTML = this.#highlightHtml(this.innerHTML)
        break
      case 'css':
        this.#shadowDom.innerHTML = this.#highlightCss(this.innerHTML)
        break
      default:
        this.#shadowDom.innerHTML = this.innerHTML
    }
    this.#shadowDom.innerHTML =
      this.#shadowDom.innerHTML + '<style>@import "/SimpleWebDesign/resources/style.css"</style>'
  }
  #highlightHtml(string) {
    ...
    for (let i = 0; i < string.length; i++) {
      if (
        ...
        !comment &&
        i + 3 < string.length &&
        string.substring(i, i + 4) == '&lt;' &&
        !(i + 7 < string.length && string.substring(i, i + 7) == '&lt;!--')
      ) {
        ...
        codeString += value
          ? string.substring(lastAddedIndex, i) + '"</span>'
          : string.substring(lastAddedIndex, i) + '<span class="green-text">"'
        value = !value
        lastAddedIndex = i + 1
      } else if (
        !tag &&
