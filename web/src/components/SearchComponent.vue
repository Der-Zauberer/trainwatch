...
const results = resource({
    parameter,
    loader: (parameter) => !parameter.name ? [] : surrealdb.up().then(() => surrealdb.query<Entity<'stop'>[]>('fn::stop::search($name)', { name: parameter.name }).then(result => result.flat().splice(0, 20)))
})
